...
def log(txt, type, bellman_update, alpha):
    fpath = './logs_two/{}/4-result_alpha_{}_{}.log'.format(RUN_FOR, alpha, type)
...
def run(bellman_update, alpha):
    ...
    for lamb in [5, 2, 0.5, 0.1, -0.1, -0.5, -2, -5]:
    #for lamb in [5]:
        if lamb == 0:
            print('skip 0')
            continue
                ...
                for j in range(0, 31):
                    ...
                    for episode in range(1, train_episodes+1):
                        ...
                        try:
                            ...
                            log('{}\t{}\t{}\t{}'.format(lamb, bellman_update, episode, 1), 'error-under', bellman_update, alpha)
                            error = True
                            break
                        except ValueError:
                            log('{}\t{}\t{}\t{}'.format(lamb, bellman_update, episode, 1), 'error-value', bellman_update, alpha)
                            error = True
                            break
                        ...
                        if math.isnan(vs0):
                            log('{}\t{}\t{}\t{}'.format(lamb, bellman_update, episode, 1), 'error-isnan', bellman_update, alpha)
                            error = True
                            break
                        ...
                        if episode == train_episodes:
                            log('{}\t{}\t{}\t{}\t{}\t{}'.format(lamb, bellman_update, episode, i, j, st.mode(calcs)), 'calc', bellman_update,
                                alpha)
