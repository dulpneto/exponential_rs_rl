...
def log(txt, type, bellman_update, alpha):
    fpath = './logs/{}/result_shape_{}_{}.log'.format(RUN_FOR, alpha, type)
...
def run(bellman_update, shape_size, alpha):
        ...
        for lamb in [-1.5,1.5]:
            ...
            for episode in range(1, train_episodes+1):
                ...
                while not done:
                    ...
                    # step
                    if total_training_rewards < (MIN_TRAIN_REWARDS*shape_size):
                    ...
                    if steps_to_update_target_model % 4 == 0 or done:
                        try:
                            ...
                            log('{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, episode, 1), 'error', bellman_update, alpha)
                            error = True
                            break
                    ...
                    if done:
                        ...
                        log('{}\t{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, episode, vs0), 'vso', bellman_update, alpha)
                        log('{}\t{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, episode, model.bellman.max_u), 'us', bellman_update, alpha)
                        log('{}\t{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, episode, model.bellman.min_u), 'us_min',
                            bellman_update, alpha)
                        log('{}\t{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, episode, total_training_rewards), 'rewards', bellman_update, alpha)
                        model.bellman.max_u = 0
                        model.bellman.min_u = sys.maxsize

                        safe_points = model.find_safe_points()
                        log('{}\t{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, episode, safe_points), 'safe', bellman_update, alpha)
            ...
            if len(policy_stack[env.s0]) > 0:
                ...
                if proper_policy:
                    log('{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, 1), 'proper',
                        bellman_update, alpha)
                else:
                    log('{}\t{}\t{}\t{}\t{}'.format(shape_size, lamb, bellman_update, sample, 1), 'not_proper',
                        bellman_update, alpha)
...
def main():
    try:
        ...
        parser.add_argument('-s', '--shape_size', type=int, default=10,
                            help='The environment shape.')
